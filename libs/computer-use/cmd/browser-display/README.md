...
- `capture`: JPEG bytes as JSON base64, actual physical size, `cursorIncluded:true`. `waitMs` (1–250) holds an unchanged capture until damage, a pointer move the capture composites, a new cursor identity or the end of a layout, then answers `{changed:false}`; a frame's `timings.waitUs` runs from the request to the moment its pixels are read, so the request's time plus `waitUs` is the frame's capture clock, and the frame's fetch, conversion and encoding are timed after it. `cursorIdentity:true` adds `cursor` to any reply whose displayed cursor differs from the one last reported: `{serial, css, members}` for a cursor the pinned Chromium draws for a CSS keyword, or `{serial, css:null, image:{hash,width,height,hotX,hotY,scale,png}}` for any other, as a PNG of at most 4 KiB at the display's scale, halved once to scale 1 when it does not fit, and `default` when it still does not. A frame answering `force:true` names the displayed cursor even when it was reported before: the driver forces a frame when it holds none for the display, which includes having dropped a frame taken across a layout together with the identity it carried; an unchanged answer never repeats one. Chromium recreates a page's own cursor, with a new serial, each time the pointer re-enters its element; the helper encodes each picture once per hash, keeping at most 32, and every later report carries the same picture, since the driver forwards records as they are and replays the newest to each viewer that connects. `visible` names the browser window inside a larger size-class framebuffer, as the display has it after the last layout, whether or not that layout's outcome is known; absent means the whole frame.
...
Every keyword record also carries `members`, the keywords its image stands for as the qualification measured them: 1–64 entries of `a-z` and `-`, within which the driver refines `css` from the hovered element's computed cursor, and which it never forwards to viewers. The arrow stands for `default` alone (`auto` computes to `default`); the `X_cursor` for `context-menu`, `help`, `vertical-text`, `alias`, `copy`, `no-drop`, `not-allowed`, `nesw-resize`, `nwse-resize`, `zoom-in` and `zoom-out`, answered as `default`, which stands when the driver can name none of them; a shared image for its class (`pointer` and `grabbing`, `progress` and `wait`, `move` and `all-scroll`, `ew-resize` and `col-resize`, `ns-resize` and `row-resize`); an image unique to one keyword for that keyword; and a cursor the helper cannot describe is `default` standing for `default`. An image record carries no `members`. The arrow and the `X_cursor` are two identities, so a move between them is reported.
