...
The browser is an ordinary workspace process. Use `agent-browser open`, `snapshot`, `click`, `fill`, `screenshot`, and the remaining CLI through `run_program`. The first command can start its daemon normally: the native session owner retains detached descendants after the command exits, so browser startup needs no separate foreground recipe. Keep the returned process identity while the browser is running. Use `agent-browser --help` and command-specific help for details. Explicit `agent-browser daemon` and `--require-daemon` remain available when a task needs foreground execution or attach-only clients. For an explicitly temporary CLI-owned browser, close it with `agent-browser close` when its work is finished and observe the owning process scope settle. Prefer the host-bound browser tools when offered: their retained session owner controls lifetime across assistant answers. Do not close that retained browser merely because an answer is complete. The existing cancellation owner stops that same scope, including detached descendants. Upstream's optional `agent-browser chat` has its own model client; Ambit does not use it or forward model-provider credentials. The existing Agent Plane remains the sole owner of model selection, reasoning, context and accounting.
...
The native Linux window owner selects Chromium's documented ANGLE software GLES driver with `--use-gl=angle --use-angle=swiftshader`. This supports WebGL1 and WebGL2 without disabling the Chromium sandbox or enabling its unsafe automatic WebGL fallback. The existing WebGPU preset and explicit browser arguments retain their own graphics choices. A fresh driver-owned window starts on `about:blank`; explicit profiles, restored sessions and custom startup targets retain their startup behavior. Opening a normal new tab through Chrome still uses Chrome's own new-tab page. The launcher does not overwrite environment or configuration arguments. This is software graphics, not a claim of hardware acceleration, WebGPU, DRM or proprietary codec support. `conformance/graphics.py` checks actual shader pixels on an ordinary HTTP page, canvas/window resize, context loss/restoration, sandbox status and the pixels in the native window stream.
