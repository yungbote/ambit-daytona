...
func NewStaticPolicyRegistry(policies []Policy) (*StaticPolicyRegistry, error) {
	...
	for index, policy := range policies {
		...
		key := policyKey(policy.Authority, policy.Image, policy.Interface, policy.Executor, policy.Executable)
	...
	sort.Slice(cloned, func(left, right int) bool {
		return policyKey(cloned[left].Authority, cloned[left].Image, cloned[left].Interface, cloned[left].Executor, cloned[left].Executable) <
			policyKey(cloned[right].Authority, cloned[right].Image, cloned[right].Interface, cloned[right].Executor, cloned[right].Executable)
	})
...
func LoadPolicyRegistry(path string) (*StaticPolicyRegistry, error) {
	...
	for index, item := range document.Policies {
		var seccomp []byte
		if item.Image.PackID == "web-browser" {
			if !filepath.IsAbs(item.SeccompPath) || !exactDigest(item.SeccompDigest) {
				return nil, fmt.Errorf("web-browser policy requires an absolute pinned seccomp profile")
			}
			seccomp, err = os.ReadFile(item.SeccompPath)
			if err != nil {
				return nil, fmt.Errorf("read web-browser seccomp profile: %w", err)
			}
			if sha256Digest(seccomp) != item.SeccompDigest {
				return nil, fmt.Errorf("web-browser seccomp profile differs from its digest")
			}
			var seccompDocument map[string]any
			if err := generationstop.DecodeCanonicalJSON(seccomp, &seccompDocument); err != nil {
				return nil, fmt.Errorf("web-browser seccomp profile is not canonical JSON: %w", err)
			}
		} else if item.SeccompPath != "" || item.SeccompDigest != "" {
			return nil, fmt.Errorf("non-browser policy cannot substitute a custom seccomp profile")
		}
		policies[index] = Policy{
			Authority: item.Authority, Image: item.Image, Interface: item.Interface, Executor: item.Executor,
...
func (registry *StaticPolicyRegistry) Resolve(request Request) (Policy, error) {
	wanted := policyKey(request.ProviderPolicy, request.Image, request.Interface, request.Executor, request.Executable)
	for _, policy := range registry.policies {
		if policyKey(policy.Authority, policy.Image, policy.Interface, policy.Executor, policy.Executable) == wanted {
...
func validatePolicy(policy Policy) error {
	...
	if policy.Image.PackID == "web-browser" {
		if len(bytes.TrimSpace(policy.Seccomp)) == 0 || policy.ShmSize <= 0 {
			return fmt.Errorf("web-browser policy requires custom seccomp and shm")
		}
	} else if len(policy.Seccomp) != 0 {
		return fmt.Errorf("non-browser policy cannot substitute custom seccomp")
	}
...
func ComputePolicyDigest(policy Policy) (string, error) {
	...
	data, err := generationstop.CanonicalJSON(policyDigestPayload{
		Schema: PolicyEntrySchema, Ref: policy.Authority.Ref,
...
func policyKey(authority Pin, image ImagePin, transport Pin, executor Pin, executable string) string {
	return authority.Ref + "\x00" + authority.Digest + "\x00" + image.Ref + "\x00" + image.ConfigDigest + "\x00" + image.PackID + "\x00" + image.PackRef +
