...
The control channel is proven against the same driver stand-in, kept answering on one connection and reporting which connection answers: replies byte for byte what the POST route answers for the same documents, failures included, in the order of frames sent before any reply was read; one driver connection across commands while a POST dials its own; the three protocol violations each closing with `1008` and their reason while a refused command is only that command's `400`; `4410 browser_view_ended` after an unclean shell exit; a peer that stops answering pings dropped while an answering peer keeps its channel; and the per-command cost of both transports, reported by the test.
